...
PDFgj 是一个 Windows 平台的 Python 命令行工具，用于图片/Word/Excel/PPT 转 PDF 以及 PDF 合并。Word/Excel/PPT 转换依赖 Office COM 自动化，仅支持 Windows；图片转 PDF 和 PDF 合并是跨平台的。
...
constants.py          (零依赖 — 文件格式集合 + COM 枚举常量)
   ...
   com_core.py        (COM 进程生命周期、HRESULT 错误分类、批量转换编排)
      ↓
   ┌──┼──┐
image word excel ppt  (转换器模块：各自通过 com_core 调用 COM，通过 utils 获取辅助函数)
   └──┼──┘
      ↓
...
**处理器模块**：
- `image_processor.py` — 图片预处理：验证、EXIF 方向矫正、GIF 第一帧提取
- `pdf_processor.py` — PDF 处理：文件有效性验证、页面尺寸标准化、方向矫正

**五种互斥模式**（通过 argparse 互斥组实现，默认为合并）：
- **PDF 合并**（默认/`-m`）：`merge.py`，使用 pypdf `PdfWriter`
- **图片转 PDF**（`-i`）：`image_convert.py` + `image_processor.py`，使用 Pillow，自动处理 RGBA/P→RGB
- **Word 转 PDF**（`-w`）：`word_convert.py`，COM 自动化 `Word.Application`
- **Excel 转 PDF**（`-e`）：`excel_convert.py`，COM 自动化 `Excel.Application`
- **PPT 转 PDF**（`-p`）：`ppt_convert.py`，COM 自动化 `PowerPoint.Application`，支持 `--slides` 指定页码范围
